...
\renewcommand{\shortauthors}{Gois and Silva}
...
\begin{quote}
\small\textit{Draft v0.5 --- 24 September 2026. Practice paper aimed at ACM DGOV, not submitted. Authorship is provisional. Diagrams: Grok Imagine, type set from team counts. Prior Sonar read: \texttt{docs/perplexity-review.md}. Venue note: \texttt{docs/journal.md}.}
\end{quote}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/grok_paper_summary.png}
\caption{Graphical abstract. Scene drawn with Grok Imagine; title, rates, and the limit set in type from the team note. Grouping rate is not precision. The leftover language-model step remains an ablation.}
\label{fig:teaser}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/grok_infographic_jobs.png}
\caption{Three jobs that public-sector NLP often mixes. Route sends a text to a unit. Theme sorts a mixed pile. Case puts two tellings of one event in one folder. Only the third job is Similaridade. Diagram: Grok Imagine; labels set in type.}
\label{fig:case}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/grok_infographic_flow.png}
\caption{Homolog path on the same incoming text: complaint, embed, HDBSCAN, leftover, officer stamp. The leftover station is an ablation; host and candidate rule are unknown. Diagram: Grok Imagine; labels set in type.}
\label{fig:pipeline}
\end{figure*}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/grok_bands.png}
\caption{Conceptual illustration (Grok Imagine). Three trays: suggested bulk link, confirm, read with care. The stamp waits on every tray. These are interface queues, not a measured safety threshold.}
\label{fig:bands}
\end{figure}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/grok_infographic_rates.png}
\caption{Three treatments on the same 4{,}389 complaints. Stack height is schematic, not to scale. Printed rates are the team counts. The middle value is inferred ($3{,}725-104$). The right-hand 84.9\% includes an undocumented leftover filter. None of the rates is a precision. Diagram: Grok Imagine; numbers set in type.}
\label{fig:rate}
\end{figure*}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/grok_funnel.png}
\caption{Conceptual illustration (Grok Imagine) of the leftover path. Counts on this batch: $768 \rightarrow 27 \rightarrow 104$ in 21 groups. The selection rule for the 27 is unknown, so the 104 cannot yet be audited.}
\label{fig:funnel}
\end{figure}
...
\item There is no officer gold, so there is no precision, recall, $\kappa$, or BCubed score.
...
\begin{acks}
ASESI colleagues who discussed the method in June--September 2026, the COUVI officers who will have to trust or reject the green queue, and Lucas Pimentel, who hosts the internal GitLab project. Errors in this draft are the authors'.
\end{acks}

\section*{CRediT (provisional)}
Conceptualization and writing (original draft): Francisco Nauber Bernardo Gois. Methodology, software, and investigation (homolog measurements): Berg Silva. Other roles to be assigned.
...
A language model helped organize this English draft from team notes. Diagrams and the graphical abstract were drawn with Grok Imagine (model grok-imagine-image-quality). Titles, rates, and captions were set in type from the team counts; the generator was not asked to write numerals. The 28 August production-day chart remains matplotlib. No complaint text was sent to an image or language model. The corresponding author is responsible for every number.
